...
> Cập nhật 2026-06-25: **pivot sang phát hiện nhị phân (benign vs malware) trên PE thô** (figshare + MalwareBazaar + RAT + benign tự thu). **Bỏ Malimg & BIG2015.** Giữ 2 kết luận chính từ lưới hợp nhất 5×3 (ảnh 3-kênh composite + hiệu quả độ phân giải 224²) và nhánh phụ phân loại họ (AVClass2 + family→behavior).
...
- **Lưới:** `{gray1, gray×3, +entropy, +ascii, full} × {224, 336, 448}` = 15 ô/model × **≥3 seed**, trên cùng tập `sweep_*.csv` (res_eligible ≥448²). Model: ResNet50 (đủ lưới) + ConvNeXt-Tiny (khẳng định trục độ phân giải với `full`).
- **Rút 2 kết luận:** (1) đọc theo hàng → composite > ảnh xám (`full > (+entropy,+ascii) > gray1 ≈ gray×3`); (2) đọc theo cột → 224² ≥/≈ 336²/448² nhưng rẻ ~2–4× (mean±std + kiểm định thống kê + bảng accuracy-vs-cost). Output: **1 bảng config×size, làm nổi ô tốt nhất**.
...
| 4 | Model | 1–2 ngày |
| 5 | Train | 4–6 ngày |
| 5b | Lưới hợp nhất 5×3 (2 kết luận) | 3–4 ngày |
| 6 | Đánh giá | 2–3 ngày |
